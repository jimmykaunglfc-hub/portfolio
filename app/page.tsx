...
export default function Home() {
  ...
  // NEW: Tabbed View State to control Native App Navigation
  const [currentView, setCurrentView] = useState('home');

  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash;
      // If the URL requests the Matrix or Experience, switch to the Matrix Tab
      if (hash === '#expertise' || hash === '#experience') {
        setCurrentView('matrix');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      } else {
        // Otherwise, default to the Home Tab
        setCurrentView('home');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
    };

    // Check the URL immediately when the app loads
    handleHashChange();

    // Listen for any navigation clicks
  ...
  return (
    <main className="w-full max-w-7xl mx-auto px-4 md:px-8 pb-4 relative z-20">
      ...
      {/* ========================================== */}
      {/* TAB 2: THE MATRIX VIEW */}
      {/* ========================================== */}
      {currentView === 'matrix' && (
        <div className="animate-in fade-in duration-500 ease-in-out pt-6">
          <CapabilitiesMatrix />
          <TrajectoryHubs />
          <StrategicNetwork />
      ...
      {/* ========================================== */}
      {/* SHARED SYSTEM MODALS (Map, Contact, Footer) */}
      {/* ========================================== */}

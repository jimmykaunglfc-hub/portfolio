...
export default function Home() {
  ...
  const coreCompetencies = [
    ...
    },
    {
      ...
      color: "text-[#c0c1ff] dark:text-[#ddb7ff]"
    },
    {
  ...
  return (
    <main className="w-full transition-colors duration-400">
      <Hero />

      {/* Capabilities Section */}
      <section id="expertise" className="py-24 px-6 md:px-12 max-w-7xl mx-auto relative z-20 pointer-events-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6 border-b border-zinc-300/60 dark:border-white/5 pb-8">
            ...
            <div key={index} className="glass-card p-8 rounded-2xl flex flex-col justify-between gap-8 group">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {item.badges.map((badge, bIdx) => (
                  <span key={bIdx} className="bg-zinc-200/40 dark:bg-white/[0.03] px-3 py-1 rounded-full text-xs text-zinc-600 dark:text-slate-400 border border-zinc-300/40 dark:border-white/5 group-hover:border-[#adc6ff]/20 transition-colors duration-300">
                    {badge}
                  </span>
                ))}
              </div>
      ...
      <section id="experience" className="py-16 px-6 md:px-12 max-w-7xl mx-auto relative z-20 pointer-events-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12 border-b border-zinc-300/60 dark:border-white/5 pb-8">
          <div>
            <h2 className="text-sm font-mono uppercase tracking-widest text-[#4d8eff] dark:text-[#adc6ff] mb-2 flex items-center gap-2">
          ...
          <div className="flex items-center gap-3 self-start sm:self-center">
            <button 
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${activePage === 1 ? 'bg-[#002e6a] dark:bg-[#adc6ff] text-white dark:text-[#002e6a] shadow-lg' : 'bg-zinc-200/60 dark:bg-white/5 text-zinc-600 dark:text-slate-400 border border-zinc-300/40 dark:border-white/10 hover:bg-zinc-300/60 dark:hover:bg-white/10'}`}
            >
              Page 1: Executive Operations
            </button>
            <button 
              onClick={() => setActivePage(2)} 
              className={`px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${activePage === 2 ? 'bg-[#002e6a] dark:bg-[#adc6ff] text-white dark:text-[#002e6a] shadow-lg' : 'bg-zinc-200/60 dark:bg-white/5 text-zinc-600 dark:text-slate-400 border border-zinc-300/40 dark:border-white/10 hover:bg-zinc-300/60 dark:hover:bg-white/10'}`}
            >
            ...
            <div key={idx} className="glass-card p-6 md:p-10 rounded-2xl grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="md:col-span-1 space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <span className="inline-block bg-zinc-200/40 dark:bg-white/5 border border-zinc-300/40 dark:border-white/10 px-2 py-0.5 rounded text-[10px] font-mono uppercase text-zinc-600 dark:text-slate-400">{block.type}</span>
                </div>
              </div>

              <div className="md:col-span-3 space-y-12 border-l border-zinc-300 dark:border-white/5 pl-6 md:pl-10">
                  ...
                  <div key={rIdx} className="space-y-4 relative group/role">
                    ...
                    <ul className="space-y-2.5 pt-2">
                      {role.bullets.map((bullet, bIdx) => (
                        <li key={bIdx} className="text-sm text-zinc-600 dark:text-slate-400 font-light flex items-start gap-2.5 leading-relaxed">

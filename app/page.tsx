...
export default function Home() {
  ...
  // HYBRID ENGINE: Controls whether to show full site (Web) or tabbed views (App)
  const [isApp, setIsApp] = useState(false);
  const [activeAppTab, setActiveAppTab] = useState('home');

  useEffect(() => {
    // 1. Detect if running inside a Native App wrapper
    ...
    const handleHashChange = () => {
      ...
      if (hash === '#expertise' || hash === '#experience') {
        ...
        window.scrollTo({ top: 0, behavior: 'instant' }); // Snap to top for app feel
      } else if (hash === '' || hash === '#contact' || hash === '#download') {
  ...
  return (
    <main className="w-full max-w-7xl mx-auto px-4 md:px-8 pb-4 relative z-20">
      ...
      {/* SECTION 1: HOME COMPONENTS 
        Hidden ONLY if running inside the App AND the Matrix tab is selected.
      */}
      <div className={isApp && activeAppTab !== 'home' ? 'hidden' : 'block animate-in fade-in duration-500'}>
      ...
      {/* SECTION 2: MATRIX COMPONENTS 
        Hidden ONLY if running inside the App AND the Home tab is selected.
      */}
      <div className={isApp && activeAppTab !== 'matrix' ? 'hidden' : 'block animate-in fade-in duration-500'}>
        <CapabilitiesMatrix />
        <TrajectoryHubs />
        <StrategicNetwork />
      ...
      {/* SHARED MODALS (Contact & Map Overlays) */}
      {isContactOpen && (

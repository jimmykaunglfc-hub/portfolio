              ...
              <div key={index} className="glass-card p-8 rounded-2xl flex flex-col justify-between gap-8 group border border-zinc-200/60 dark:border-zinc-800/40">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {item.badges.map((badge, bIdx) => (
                    <span key={bIdx} className="bg-zinc-100 dark:bg-white/[0.03] px-3 py-1 rounded-full text-xs text-zinc-600 dark:text-slate-400 border border-zinc-200 dark:border-white/5 group-hover:border-[#adc6ff]/20 transition-colors duration-300">
                      {badge}
                    </span>
                  ))}
                </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12 border-b border-zinc-200 dark:border-white/5 pb-8">
            ...
            <div className="flex items-center gap-3 self-start sm:self-center">
              <button 
                ...
                className={`px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${activePage === 1 ? 'bg-[#002e6a] dark:bg-[#adc6ff] text-white dark:text-[#002e6a] shadow-lg' : 'bg-zinc-100 dark:bg-white/5 text-zinc-600 dark:text-slate-400 border border-zinc-200 dark:border-white/10 hover:bg-zinc-200 dark:hover:bg-white/10'}`}
              >
                Page 1: Executive Operations
              </button>
              <button 
                onClick={() => setActivePage(2)} 
                className={`px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${activePage === 2 ? 'bg-[#002e6a] dark:bg-[#adc6ff] text-white dark:text-[#002e6a] shadow-lg' : 'bg-zinc-100 dark:bg-white/5 text-zinc-600 dark:text-slate-400 border border-zinc-200 dark:border-white/10 hover:bg-zinc-200 dark:hover:bg-white/10'}`}
              >
          ...
          <div className="space-y-12 transition-all duration-500">
            {paginatedExperience[activePage as 1 | 2].map((block, idx) => (
              <div key={idx} className="glass-card p-6 md:p-10 rounded-2xl grid grid-cols-1 md:grid-cols-4 gap-8 border border-zinc-200/60 dark:border-zinc-800/40">
                ...
                <div className="md:col-span-3 space-y-12 border-l border-zinc-200 dark:border-white/5 pl-6 md:pl-10">
                  {block.roles.map((role, rIdx) => (
                    <div key={rIdx} className="space-y-4 relative group/role">
                      <div className="absolute -left-[31px] md:-left-[47px] top-2 w-2.5 h-2.5 rounded-full bg-zinc-200 dark:bg-zinc-800 border-2 border-[#4d8eff] dark:border-[#adc6ff] group-hover/role:bg-[#adc6ff] transition-colors duration-300" />
                      ...
                      {'skills' in role && (
                        <div className="flex flex-wrap gap-2 pt-3">
                          {role.skills?.map((skill, sIdx) => (
                            <span key={sIdx} className="bg-zinc-100 dark:bg-blue-500/5 text-[#4d8eff] dark:text-[#adc6ff] border border-zinc-200 dark:border-[#adc6ff]/10 px-2.5 py-1 rounded text-xs font-mono">
                              {skill}
                            </span>
                          ))}
                        </div>

...
import { masterQuestionBank } from "./questions"; // <-- We now import from your data file!
  ...
  return (
    <main className="min-h-screen pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto relative z-10 font-sans">
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {!isGameOver && (
          <button onClick={quitGame} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-red-500/10 text-red-600 dark:text-red-400 font-bold text-sm hover:bg-red-500/20 transition-colors">
          ...
          <div className="w-full aspect-square rounded-3xl p-4 bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/10 shadow-2xl relative overflow-hidden flex items-center justify-center">
            ...
            <div className="relative z-10 grid grid-cols-3 gap-2 w-full h-full">
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((index) => {
                ...
                const isFailed = index < currentQuestion && !isUnlocked;
                ...
                return (
                  <motion.div
                    ...
                    className={`rounded-xl border flex items-center justify-center backdrop-blur-md transition-all duration-500 ${!isUnlocked && "bg-gray-100 dark:bg-zinc-900 shadow-inner"}`}
                  >
          ...
          <AnimatePresence mode="wait">
            {isGameOver ? (
              ...
              >
                ...
                <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2">System Diagnostics Complete</h2>
                <p className="text-gray-600 dark:text-gray-400 mb-8 text-lg">You successfully unlocked {score} out of 9 matrix nodes.</p>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <button onClick={startNewGame} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-blue-600 text-white px-8 py-3.5 rounded-full font-bold tracking-wide hover:bg-blue-500 transition-colors">
                    <RefreshCcw className="w-5 h-5" /> Replay Matrix
                  </button>
                  <button onClick={() => router.push('/games')} className="w-full sm:w-auto bg-gray-200 dark:bg-white/10 text-gray-900 dark:text-white px-8 py-3.5 rounded-full font-bold tracking-wide hover:bg-gray-300 dark:hover:bg-white/20 transition-colors">
                    Return to Arcade
                  </button>
                </div>
              </motion.div>
                ...
                <div className="space-y-3">
                  {activeQuestions[currentQuestion].options.map((option, index) => {
                    ...
                    if (selectedAnswer !== null) {
                      if (isCorrectAnswer) {
                        btnStyle = "bg-green-500/10 border-green-500/50 text-green-600 dark:text-green-400";
                      } else if (isSelected && !isCorrectAnswer) {
                        btnStyle = "bg-red-500/10 border-red-500/50 text-red-600 dark:text-red-400";
                      } else {
                        btnStyle = "opacity-40 pointer-events-none border-transparent dark:border-transparent";
                      }
                    }

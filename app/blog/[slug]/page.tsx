...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  // Fetch the flat text body directly from Sanity
  ...
  // Smart UI text parser engine
  const renderContent = (rawText: string) => {
    if (!rawText) return null;
    
    // Split text into paragraphs/sections by double newlines
    const blocks = rawText.split('\n\n');

    return blocks.map((block, index) => {
      const trimmed = block.trim();

      // 1. Render H3 Headings (###)
      ...
      // 2. Render Highlighted Callout/Quote Blocks (>)
      if (trimmed.startsWith('>')) {
        return (
      ...
      // 4. Render Normal Body Paragraphs
      return (
  ...
  return (
    <main className="min-h-screen bg-[#030303] text-slate-200 font-sans pt-32 pb-24 selection:bg-blue-500/20">
      <div className="max-w-3xl mx-auto px-4 md:px-6">
        ...
        {/* Title Header with Gradient Accent Split */}
        ...
        {/* Dynamically Processed Text Content Output */}
